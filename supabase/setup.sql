...
create policy "public_insert_newsletter" on public.newsletter_subscriptions
for insert to public
with check (true);
...
-- Admin read access for newsletter + analytics
create policy "admin_read_newsletter" on public.newsletter_subscriptions
for select to authenticated
using (exists (select 1 from public.admin_users a where a.id = auth.uid()));
